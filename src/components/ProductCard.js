    ...
    return (
        <div className="group relative flex flex-col bg-white border border-gray-100 rounded-[2rem] p-3 transition-all duration-500 hover:shadow-2xl hover:shadow-gray-100/50">
            ...
            >
                ...
                {/* Variant Swipe Overlay */}
                <div className="absolute inset-x-0 bottom-0 p-4 translate-y-full group-hover:translate-y-0 transition-transform duration-500 bg-gradient-to-t from-black/20 to-transparent flex justify-center gap-2">
                  {product.variants.map((v) => (
                    <button 
                      key={v.name}
                      onClick={(e) => { e.preventDefault(); setSelectedVariant(v); }}
                      className={`w-2 h-2 rounded-full transition-all ${selectedVariant.name === v.name ? 'bg-white w-6' : 'bg-white/60 hover:bg-white'}`}
                    />
                  ))}
                </div>
            </Link>
            ...
            <div className="space-y-4 px-2 pb-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {product.features?.slice(0, 2).map((f) => (
                    <span key={f} className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">{f}</span>
                  ))}
                </div>
                ...
                <div className="flex items-center gap-2 pt-2">
                    <button
                        ...
                        className="flex-1 bg-gray-900 text-white text-xs font-bold uppercase tracking-wider py-3.5 rounded-xl hover:bg-orange-600 transition-all active:scale-95 flex items-center justify-center gap-2"
                    >
                        Buy Now
                    </button>
                    <button
